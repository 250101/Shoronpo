...
$migration = Join-Path (Split-Path $PSScriptRoot -Parent) 'supabase\migrations\0013_tspoonlab_stock_sync.sql'

if (-not (Test-Path -LiteralPath $psql)) { throw 'No se encontro psql.' }
if (-not (Test-Path -LiteralPath $migration)) { throw 'No se encontro la migracion de Fase 5.' }
...
try {
  ...
  if ($LASTEXITCODE -ne 0) { throw 'La migracion de Fase 5 no pudo aplicarse.' }
  Write-Host 'OK: esquema de Fase 5 aplicado.' -ForegroundColor Green
} finally {
