...
$migration=Join-Path (Split-Path $PSScriptRoot -Parent) 'supabase\migrations\0015_tspoonlab_productions.sql'
...
try{
  ...
  if($LASTEXITCODE-ne 0){throw 'No se pudo aplicar el esquema de producciones.'}
  Write-Host 'OK: esquema de producciones aplicado.' -ForegroundColor Green
}finally{
